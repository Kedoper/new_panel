...
function preRenderReportCart(id, date, price) {
    let block = `<div class="report-card" data-repid="${id}">
                ...
                <div class="report-card__buttons">
                    <button type="button" onclick="openFullReport(${id})">Подробнее</button>
                </div>
            </div>`;
    return block;
}
...
function renderRecentReports(data) {
    ...
    data.forEach(function (item) {
        items = items + preRenderReportCart(item.report_id, item.report_datetime, item.report_price);
    });
    $('.recent-reports-block').html(items);
}

$(document).ready(function () {
    $.ajax({
        type: "POST",
        url: "/engine/handlers/loaders/loadRecentReports.php",
